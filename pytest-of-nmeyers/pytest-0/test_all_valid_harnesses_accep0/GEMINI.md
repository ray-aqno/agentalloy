...
A local skillsmith service runs at `http://localhost:8000` with a curated corpus of engineering skills (testing, error handling, deployment, observability, security, etc.).
...
Health check: `curl -s http://localhost:8000/health` → `{"status":"ok"}`. If unreachable, run `skillsmith doctor` from the skillsmith repo.
