...
Handles: ::note, :::caution, :::warning, <Tabs>, <TabItem>, <Figure>,
         ...
         <import />, <CopilotBeta />, <a>, <Link>, <Lightbox>
"""
...
def strip_jsx(text: str) -> str:
    ...
    # The YAML file uses literal backslash-quote (\") in href attributes.
    # Use (?:\\"|") to match both \" (backslash-escaped) and " (regular) quotes.
    # First handle paired <a>...</a> tags
    text = re.sub(
        r'<a\s+href=(?:\\"|")[^\\"]*(?:\\"|")[^>]*>(.*?)</a>', r"\1", text, flags=re.DOTALL
    )
    # Then handle standalone opening <a> tags (no closing </a>)
    text = re.sub(r'<a\s+href=(?:\\"|")[^\\"]*(?:\\"|")[^>]*>', "", text, flags=re.DOTALL)
    ...
    # The YAML file uses literal backslash-quote in href attributes
    text = re.sub(
        r'<Link\s+href=(?:\\"|")[^\\"]*(?:\\"|")[^>]*>(.*?)</Link>', r"\1", text, flags=re.DOTALL
    )
    text = re.sub(r'<Link\s+href=(?:\\"|")[^\\"]*(?:\\"|")[^>]*>', "", text, flags=re.DOTALL)

    # 18. Handle leftover fragments like `.yml'>` on their own line
    # Match the newline before the artifact, the artifact, and the newline after
    text = re.sub(r"\n\.yml\'\s*>\s*\n", "\n", text)
    # Also handle inline remnants (no surrounding newlines)
    text = re.sub(r"\s+\.yml\'\s*>", "", text)
    # Handle bare .yml'> with no surrounding whitespace
    text = re.sub(r"\.yml\'\s*>", "", text)
